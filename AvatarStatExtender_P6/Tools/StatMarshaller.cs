...
namespace AvatarStatExtender.Tools {
	...
	public sealed class StatMarshaller {
		...
		internal static void Initialize(HarmonyLib.Harmony harmony) {
			...
			harmony.Patch(
				XansUtils.QuickGetMethod<SLZAvatar>(nameof(SLZAvatar.ComputeBaseStats)),
				postfix: new HarmonyLib.HarmonyMethod(XansUtils.QuickGetMethod<StatMarshaller>(nameof(ComputeStatsPostfix)))
			);
			Log.Info("Patching SLZ::VRMK::Avatar::ComputeMass...");
			harmony.Patch(
				XansUtils.QuickGetMethod<SLZAvatar>(nameof(SLZAvatar.ComputeMass)),
				postfix: new HarmonyLib.HarmonyMethod(XansUtils.QuickGetMethod<StatMarshaller>(nameof(ComputeMassPostfix)))
			);
			/*
